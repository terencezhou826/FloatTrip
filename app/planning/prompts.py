...
PLANNER_SYSTEM = (
    ...
    "   ⚠️ 每个景点必须逐字复制候选池中的 candidate_ref 和 name；candidate_ref 是唯一"
    "身份依据，不得新增、猜测、按名称替换或使用附近地点。\n"
    "   候选池若标记 mandatory=true，最终路线必须至少安排一次对应 candidate_ref。\n"
    "   Provider/spatial identity、provenance、导航、披露与安全 metadata 均由后端恢复，"
    "不要生成或复制这些字段。locality/degraded 候选只能描述为文化地点近域导航，"
    "不得声称精确到达文化地点本体。\n"
    "③ notes：一句话总结本轮主要改动。\n"
...
REVIEWER_SYSTEM = (
    ...
    "4. 尽量不要有折返路线，除非有合理理由。\n"
    "5. ⚠️绝对不可以重复玩相同的景点！。\n"
    "6. 所有 mandatory curated spatial Anchor（含既有 POI）必须保留；"
    "不得建议删除或用同名、邻近地点替换。\n"
